    ...
    for row in spamreader:
        ...
        if rowCount == 0:
            try:
                ...
                logFile.write("FATAL ERROR: There is an error in the names for the columns for the userdata.")
                logFile.close()
            ...
            try:
                ...
                logFile.write("FATAL ERROR: You forgot to add the column with invoice numbers called: Factuurnummer.")
                logFile.close()
            ...
            try:
                ...
                logFile.write("FATAL ERROR: You forgot to add the column with the subject called: Onderwerp.")
                logFile.close()
            ...
            try:
                ...
                logFile.write("FATAL ERROR: You forgot to add the column with description for the invoice called: Zaak.")
                logFile.close()
            ...
            try:
                ...
                logFile.write("FATAL ERROR: You forgot to add the column with the amounts of the bills called: Bedrag.")
                logFile.close()
            ...
            if termijnIndexPresent == true:
                termijn = row[termijnIndex].split(";")
            '''
            Excel tends to mess up the invoice numbers by writing them in scientific notation or deleting the prepended zeroes. This codes attempts to restore that.
            '''
            try:
            ...
            '''
            By using regex the template keywords get replaced by their proper values. As the posts do not need to be added to the .csv, this codes checks for its presence and ensures all has been filled in correctly.
            '''
            ...
            if termijnIndexPresent == true:
                try:
                    newFileContent = newFileContent.replace("termijn}{termijn","termijn}{"+int(termijn))
                except:
                    print "ERROR: There is something wrong with the payment period on row: "+str(rowCount)+"."
                    logFile("ERROR: There is something wrong with the payment period on row: "+str(rowCount)+". \n")
            '''
            ...
            '''
            newFile = open(destinationFolder+"/"+onderwerp+" "+voornaam+" "+achternaam+".tex","a+")
            newFile.write(newFileContent)
            newFile.close()
            create_pdf(destinationFolder+"/"+onderwerp+" "+voornaam+" "+achternaam+".tex","Testen/"+onderwerp+" "+voornaam+" "+achternaam)
#            os.remove(destinationFolder+"/"+onderwerp+" "+voornaam+" "+achternaam+".tex")
            rowCount = rowCount+1
'''
Garbage collection and show log file on exit. 
'''
logFile.close()
