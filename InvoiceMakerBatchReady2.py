                ...
                for row in spamreader:
                    ...
                    if rowCount == 0:
                        ...
                        try:
                            ...
                            logFile.write(
                                "NOTICE:  You did not add a column for the entries, the subject will be used as the entry. \n")
                        rowCount = rowCount + 1
                        '''
                        ...
                        newFileContent = templateContent.replace(
                            "adres}{adres", "adres}{" + initialen + " " + achternaam + r'\\' + adres + r'\\' + postcode + " " + plaats)
                        try:
                            if self.CheckVar4.get() == 1:
                                ...
                                if len(factuurnummer) + 1 != int(self.EntryVar.get()):
                                    ...
                                    try:
                                        numberFormat = "{0:0" + \
                                            self.EntryVar.get() + "d}"
                                        factuurnummer = numberFormat.format(
                                            int(factuurnummer))
                        ...
                        newFileContent = newFileContent.replace(
                            "factuurnummer}{factuurnummer", "factuurnummer}{" + factuurnummer)
                        newFileContent = newFileContent.replace(
                            "betreftregel}{onderwerp", "betreftregel}{" + onderwerp)
                        newFileContent = newFileContent.replace(
                            "voornaam}{voornaam", "voornaam}{" + voornaam)
                        newFileContent = newFileContent.replace(
                            "zaak}{zaak", "zaak}{" + zaak)
                        if postIndexPresent == True:
                            try:
                                ...
                                for i in range(0, len(bedrag)):
                                    ...
                                    bedragLine = bedragLine + \
                                        post[i] + r" & \euro & " + self.numberCheckOn2Decimal(
                                            bedrag[i], rowCount) + r" \\[6pt]\hline "
                                    totaalbedrag = totaalbedrag + \
                                        float(bedrag[i])
                                newFileContent = newFileContent.replace(
                                    r"\betreftregel & \euro & bedrag \\[6pt]\hline", bedragLine)
                            except:
                                try:
                                    for i in range(0, len(bedrag)):
                                        bedragLine = bedragLine + r"\betreftregel & \euro & " + \
                            ...
                            try:
                                ...
                                if len(bedrag) != 1:
                                    ...
                                    logFile.write("WARNING: You wrote multiple amounts eventhough there are no different entries on row: " + str(
                        ...
                        if self.CheckVar2.get() == 1:
                            newFileContent = newFileContent.replace(
                                "paymentperiod}{14", "paymentperiod}{" + self.EntryVar2.get())
                        totaalbedrag = self.numberCheckOn2Decimal(
                            totaalbedrag, rowCount)
                        newFileContent = newFileContent.replace(
                        ...
                        '''
                        # change this to something more general and usefull as
                        # this doenst really work with Mailmerge
                        if tussenvoegsel == "":
                            newFile = open(self.destinationFolder + "/" + onderwerp +
                                           " " + voornaam + " " + achternaam + ".tex", "a+")
                            newFile.write(newFileContent)
                            newFile.close()
                            self.create_pdf(self.destinationFolder + "/" + onderwerp + " " + voornaam + " " + achternaam +
                                            ".tex", self.destinationFolder + "/" + onderwerp + " " + voornaam + " " + achternaam)
                        else:
                            newFile = open(self.destinationFolder + "/" + onderwerp + " " +
                                           voornaam + " " + tussenvoegsel + " " + achternaam + ".tex", "a+")
                            newFile.write(newFileContent)
                            newFile.close()
                            self.create_pdf(self.destinationFolder + "/" + onderwerp + " " + voornaam + " " + tussenvoegsel + " " + achternaam +
                                            ".tex", self.destinationFolder + "/" + onderwerp + " " + voornaam + " " + tussenvoegsel + " " + achternaam)
