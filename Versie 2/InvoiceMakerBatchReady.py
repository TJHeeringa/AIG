...
with open(csvFile, 'rb') as csvfile:
    ...
    for row in spamreader:
        ...
        if rowCount == 0:
            ...
            zaak = row[zaakIndex]
            bedrag = row[bedragIndex].split(blockDelimiterString)
            if tussenvoegsel != "":
                newFileContent = templateContent.replace("adres}{adres","adres}{"+initialen+" "+tussenvoegsel+" "+achternaam+r'\\'+adres+r'\\'+postcode+" "+plaats)
            else:
            ...
            if postIndexPresent == True:
                try:
                    post = row[postIndex].split(blockDelimiterString)
                    ...
                    try:
                        for i in range(0,len(bedrag)):
                            ...
                            logFile.write(str(i)+"\n")
                            logFile.write(str(bedrag)+"\n")
                            logFile.write(str(bedrag[i])+"  "+str(rowCount)+"\n")
                            totaalbedrag = totaalbedrag + float(bedrag[i])
                        newFileContent = newFileContent.replace(r"\betreftregel & \euro & bedrag \\[6pt]\hline",bedragLine)
            ...
            newFile.close()
            create_pdf(destinationFolder+"/"+onderwerp+" "+voornaam+" "+achternaam,destinationFolder+"/"+onderwerp+" "+voornaam+" "+tussenvoegsel+" "+achternaam)
            rowCount = rowCount+1
logFile.close()
